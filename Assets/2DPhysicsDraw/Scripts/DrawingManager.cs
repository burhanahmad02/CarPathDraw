...
public class DrawingManager : MonoBehaviour
{
    ...
    public List<Vector2> newVerticies2 = new List<Vector2>();
    public List<Vector2> newVerticies_ = new List<Vector2>();
    ...
    public bool canDraw; // enables or disables the drawing when clicked on some environment object,
    ...
    public Material countrysidemat;
    ...
    public void Start()
    {
       // newVerticies.Add(new Vector2(207.946167f,262.47168f));
        DestroyLastPath();
        startMoving = false;
        canDraw = true;
    ...
    public void StartVehicle()
    {
        ...
        StartCar.GetComponent<Button>().enabled = true;
        StartCar.GetComponent<Animation>().enabled = true;
    }
...
Mathf.Infinity, layerMask);
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (canDraw == true)
            {
                ...
                else
                {
                    ...
                    Debug.Log(newVerticies2);
                    
                    Debug.Log(pathPolygonCollider.points = newVerticies2.ToArray());
                    if (pathPolygonCollider.sharedMaterial != null)
                    {
                        pathPolygonCollider.sharedMaterial = null;
                    }
                    else
                    {

                        
                        Debug.Log(  pathPolygonCollider.sharedMaterial = material);
                        
                    }
                   
                }
                CalculatesPrevCenterOfMassAndMass(newVerticies.Count - 1);
               // clone.layer = LayerMask.NameToLayer("onDraw");
    ...
    void StarPrefab()
    {
        ...
        clone = Instantiate(path, mousePointer.transform.position, Quaternion.identity); // instantiate prefab
    ...
    void DrawVisibleLine()
    {
      ...
      // Debug.Log("here lies the problem"+mousePointer.transform.position);
        if (Vector2.Distance(mousePointer.transform.position, newVerticies_[posCount]) >= verticesDistance)
        {
         Debug.Log("The distance"+Vector2.Distance(mousePointer.transform.position, newVerticies_[posCount]));
            ...
            Particle.transform.position = mousePointer.transform.position - new Vector3(0, 0, Camera.main.transform.position.z);
            newVerticies_.Add(mousePointer.transform.position - clone.transform.position);
            if (mouseHit == false)
            {
                newVerticies.Add(mousePointer.transform.position - clone.transform.position);

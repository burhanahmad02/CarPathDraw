...
public class DestroyPath : MonoBehaviour {
    ...
    public static bool IntantDestroy;
    ...
    void Start () {
        ...
        IntantDestroy = false;
        GetComponent<LineRenderer>().material = lineMat[GameManager.EnvirenmentNo];
    }
	
	void Update () {

        if (IntantDestroy == true)
        {
        ...
        if (destroyCounter > 0 && isPermanent == false && canDestroy == true)
        {
            ...
            if (destroyCounter <= 0)
            {
                DrawingManager.Instance.paths.Remove(this.gameObject);
                Destroy(this.gameObject);
